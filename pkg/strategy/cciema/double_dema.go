...
// 交叉信号
func (dd *DoubleDema) getDemaCrossSignal(openPrice float64, closePrice float64) types.Direction {
	var demaSignal types.Direction = types.DirectionNone

	crossOver := types.CrossOver(dd.fastDEMA, dd.slowDEMA)
	crossUnder := types.CrossUnder(dd.fastDEMA, dd.slowDEMA)
	...
	fmt.Printf("dema fast %.4f,slow %.4f signal %d \n", dd.fastDEMA.Last(), dd.slowDEMA.Last(), demaSignal)
