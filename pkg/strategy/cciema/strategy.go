...
func (s *Strategy) SetupIndicators(store *bbgo.MarketDataStore) {
	...
	s.doubleDema = newDoubleDema(store, s.Interval, s.WindowDemaQuick, s.WindowDemaSlow)
	...
	if klines, ok := store.KLinesOfInterval(s.Interval); ok {
		s.cci.CalculateAndUpdate((*klines)[0:])
	}

	store.OnKLineWindowUpdate(func(interval types.Interval, window types.KLineWindow) {
		//fmt.Println("fuck cci1,", interval, s.cci.Interval)
		//if interval == s.cci.Interval {
		//	if len(s.cci.TypicalPrice) == 0 {
		//		for _, k := range window {
		//			fmt.Println("fuck cci2 ", k.String())
		//			s.cci.Update(k.High.Add(k.Low).Add(k.Close).Div(three).Float64())
		//
		//		}
		//	} else {
		//		k := window[len(window)-1]
		//		s.cci.Update(k.High.Add(k.Low).Add(k.Close).Div(three).Float64())
		//
		//	}
		//}
	...
	if s.LinearRegression != nil {
		if s.LinearRegression.Window == 0 {
			...
			if klines, ok := store.KLinesOfInterval(s.LinearRegression.Interval); ok {
...
func (s *Strategy) shouldStop(kline types.KLine, demaCrossSignal types.Direction, lgSignal types.Direction) bool {
	stopNow := false
	base := s.Position.GetBase()
	baseSign := base.Sign()
	fmt.Println("base,basesign", base, baseSign)
	...
	if s.StopLossByTriggeringK && !s.currentStopLossPrice.IsZero() && ((baseSign < 0 && kline.GetClose().Compare(s.currentStopLossPrice) > 0) || (baseSign > 0 && kline.GetClose().Compare(s.currentStopLossPrice) < 0)) {
		...
		stopNow = true
	} else if s.StopByReversedDema && ((baseSign < 0 && demaCrossSignal == types.DirectionUp) || (baseSign > 0 && demaCrossSignal == types.DirectionDown)) {
...
func (s *Strategy) getSide(demaCrossSignal types.Direction, demaSignal types.Direction, lgSignal types.Direction) types.SideType {
	...
	//	side = types.SideTypeSell
	//}
	if demaSignal == types.DirectionUp || demaCrossSignal == types.DirectionUp {
		side = types.SideTypeBuy
	} else if demaSignal == types.DirectionDown || demaCrossSignal == types.DirectionDown {
		side = types.SideTypeSell
	}
...
func (s *Strategy) Run(ctx context.Context, orderExecutor bbgo.OrderExecutor, session *bbgo.ExchangeSession) error {
	...
	//cci := indicator.CCI{IntervalWindow: types.IntervalWindow{Window: 34}}

	//session.MarketDataStream.OnKLineClosed(types.KLineWith(s.Symbol, types.Interval1m, func(kline types.KLine) {
	//	fmt.Println(kline.Closed)
	//
	//	cci.PushK(kline)
	//	fmt.Printf("  cci closed%.4f \n", cci.Last())
	//
	//}))
	session.MarketDataStream.OnKLine(types.KLineWith(s.Symbol, s.Interval, func(kline types.KLine) {
		...
		s.realCci = s.cci.RealPushK(kline)

		// Get order side
		side := s.getSide(demaCrossSignal, demaSignal, lgSignal)
		fmt.Printf("kline o:%.4f,c %.4f,start: %s- %s \n", kline.Open.Float64(), kline.Close.Float64(), kline.StartTime, kline.EndTime)
		fmt.Printf("atr:%.4f,deam %d,deamcross:%d ,lg:%d side %s\n", s.atr.Last(), demaSignal, demaCrossSignal, lgSignal, side)
		fmt.Printf("cci: %.4f,%.4f,%.4f,%.4f", s.realCci, s.cci.Index(0), s.cci.Index(1), s.cci.Index(2))

		// TP/SL if there's non-dust position and meets the criteria
		//if !s.Market.IsDustQuantity(s.Position.GetBase().Abs(), closePrice) && s.shouldStop(kline, demaCrossSignal, lgSignal) {
	...
	session.MarketDataStream.OnKLine(types.KLineWith(s.Symbol, s.Interval, func(kline types.KLine) {
		...
		// Get order side
		side := s.getSide(demaCrossSignal, demaSignal, lgSignal)
		fmt.Printf("s.realCci %.4f ,atr:%.4f,deam %d,deamcross:%d ,lg:%d side %s\n", s.realCci, s.atr.Last(), demaSignal, demaCrossSignal, lgSignal, side)
		//
		//fmt.Println(demaCrossSignal, demaSignal, lgSignal, side)
		...
		if side == types.SideTypeBuy {
			...
			if s.TakeProfitAtrMultiplier > 0 {
				s.currentTakeProfitPrice = closePrice.Add(fixedpoint.NewFromFloat(s.atr.Last() * s.TakeProfitAtrMultiplier))
			}
			...
			if s.TakeProfitAtrMultiplier > 0 {
				s.currentTakeProfitPrice = closePrice.Sub(fixedpoint.NewFromFloat(s.atr.Last() * s.TakeProfitAtrMultiplier))
			}
		}
